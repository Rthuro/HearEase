...
export function RecentCaseRecords({cases, user}) {
    ...
    return (
        <div className="flex items-center flex-wrap gap-3 ">
            {cases?.length > 0 ? (
                    cases?.map((caseItem) => (
                        <Link to={`/${user}/Case/${caseItem.case_number}`} key={caseItem.case_number} className="border border-zinc-200 rounded-lg p-4 w-60 hover:shadow-md transition-shadow">
