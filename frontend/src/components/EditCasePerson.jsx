...
import { updateCasePersonById } from "@/store/useRetrieveUsersStore";

export function EditCasePerson({person_info, refresh}) {
    const { barangays, streets } = useAddressesStore();
    const [openCalendar, setOpenCalendar] = useState(false);
    const minDate = new Date("1900-01-01");
    const maxDate = new Date();
    const [info, setInfo] = useState(person_info);
    const [open, setOpen] = useState(false);
    const [loader, setLoader] = useState(false);

    const handleSubmit = async (e)  => {
        e.preventDefault();
        toast.promise(updateCasePersonById(info.id, info), {
            loading: "Updating case person information...",
            success: (res) => {
                refresh();
                setOpen(false);
                return "Case person information updated successfully!";
                },
            error: "Failed to update case person information.",
        });
    }
    ...
    return (
        ...
        <DialogContent className={cn('w-full md:w-2/3')}>
                <DialogHeader>
                    <DialogTitle>Edit Case Person</DialogTitle>
                    <DialogDescription>Edit Case Person information.</DialogDescription>
                </DialogHeader>
